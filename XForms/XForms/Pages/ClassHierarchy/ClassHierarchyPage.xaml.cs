...
namespace XForms.Pages.ClassHierarchy
{
    public partial class ClassHierarchyPage : ContentPage
    {
        public ClassHierarchyPage()
        {
            ...
            GetPublicTypes(typeof (Xamarin.Forms.Xaml.Extensions).GetTypeInfo().Assembly, classList);
            ...
            // Now sort the list.
            classList.Sort((t1, t2) => { return String.Compare(t1.Type.Name, t2.Type.Name); });
        ...
        void AddItemToStackLayout(ClassAndSubclasses parentClass, int level)
        {
            ...
            if (typeInfo.IsGenericType)
            {
                ...
                {
                    name += parameters[i].Name; if (i < parameters.Length - 1) { name += ", "; }
                }
                name += ">";
            }

            // Create Label and add to StackLayout.
            Label label = new Label { Text = String.Format("{0}{1}", new string(' ', 4 * level), name), TextColor = parentClass.Type.GetTypeInfo().IsAbstract ? Color.Accent : Color.Default };
